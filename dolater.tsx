  ...
  async ({ event, step, publish }) => {
    ...
    for (let i = 0; i < analysis.screens.length; i++) {
      ...
      await step.run(`generated-screen-${i}`, async () => {
        const variantsCountRaw =
          typeof preferences?.variantsCount === "number" ? preferences.variantsCount : undefined;
        const variantsCount = Math.max(2, Math.min(5, variantsCountRaw ?? 3));
      ...
      `.trim(),
          ...
          html = (match ? match[0] : html).replace(/```/g, "");

          const { object: scoreObj } = await generateObject({
            model: "google/gemini-3-pro-preview",
            schema: AestheticScoreSchema,
            system:
              "You are an expert mobile UI aesthetic judge. Score 0–10 based on beauty, layout clarity, visual hierarchy, color harmony with theme variables, brand alignment, modern patterns, and overall polish. Return JSON only.",
            prompt: `
            THEME CSS VARIABLES:
            ${fullThemeCSS}
            
            BRAND KIT:
            Primary: ${brandKit?.primaryColor || "None"}
            Font Family: ${brandKit?.fontFamily || "None"}
            Logo URL: ${brandKit?.logoUrl || "None"}
            
            HTML TO EVALUATE:
            ${html}
            
            Provide numeric score (0–10) and short reasons.
            `.trim(),
          });
        ...
        const finalHtml = candidates[0]?.html || "";
